...
export function RouteCanvas({
    ...
    const fetchRoute = async () => {
      ...
      try {
        ...
        if (data.code === "Ok" && data.routes && data.routes.length > 0) {
          ...
          console.log(
            `[v0] ✅ Route fetched: ${distanceInMiles.toFixed(0)} miles, ${coords.length} points`,
          );
        } else {
  ...
  useEffect(() => {
    ...
    const projectPoint = (lat: number, lng: number): [number, number] => {
      ...
      const y =
        height -
        padding -
        ((lat - minLat) / (maxLat - minLat)) * (height - 2 * padding);
      return [x, y];
    };
  ...
  if (!origin || !destination) {
    return (
      <div className="bg-muted/30 text-muted-foreground flex h-48 w-full items-center justify-center rounded-lg text-sm">
        <div className="text-center">
          <MapPin className="text-muted-foreground/50 mx-auto mb-2" size={32} />
  ...
  return (
    <div className="space-y-2">
      <canvas ref={canvasRef} className="bg-muted/30 h-48 w-full rounded-lg" />
      <div className="text-muted-foreground flex items-center justify-center gap-4 text-sm">
        <div className="flex items-center gap-2">
          ...
          <span>
            {isLoadingRoute
              ? "Calculating route..."
              : `Estimated Distance: ${distance.toFixed(0)} miles`}
          </span>
        </div>
