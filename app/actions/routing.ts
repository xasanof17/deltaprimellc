...
export async function getRoute(origin: RoutePoint, destination: RoutePoint): Promise<RouteResponse> {
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.paths && data.paths.length > 0) {
          ...
          return {
            coordinates: path.points.coordinates.map((coord: number[]) => [coord[1], coord[0]]) as [number, number][],
            distance: path.distance,
            duration: path.time / 1000, // Convert ms to seconds
          }
        }
      } else if (response.status === 400) {
        console.log("GraphHopper: No driving route available between locations")
      } else {
  ...
  let errorMessage = "No driving route available - showing straight-line distance"

  if (distanceInMiles > 500) {
    errorMessage = "Locations too far apart for driving route - showing straight-line distance"
  }
